...
import type { ProposalDetailDTO, ProposalRequestDTO, ProposalSummaryDTO } from "@/types/Proposal";
...
export const proposalApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        updateProposalById: builder.mutation<ProposalSummaryDTO, { id: string; updatedProposal: Partial<ProposalRequestDTO> }>({
        ...
        deleteProposalById: builder.mutation<{ success: boolean; id: string }, string>({
            query: (id) => ({
                url: `/proposals/${id}`,
                method: "DELETE",
            }),
